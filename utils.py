...
from modeling_discrete_vae import Dalle_VAE, DiscreteVAE, VQ_GAN
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            ...
            # except:
            #     msg = model_without_ddp.load_state_dict(checkpoint['module'])
            print(msg)
...
def create_d_vae(weight_path, d_vae_type, image_size, device):
    if d_vae_type == "dall-e":
        ...
        return get_vq_gan(weight_path, image_size, device)
    elif d_vae_type == "vqgan_ema":
        return get_vq_gan(weight_path, image_size, device, is_gumbel=False)
...
def get_vq_gan(weight_path, image_size, device, is_gumbel=True):
    model = VQ_GAN(image_size, is_gumbel=is_gumbel)
    model.load_model(model_dir=weight_path, device=device)
    return model
