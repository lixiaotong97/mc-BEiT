...
model.backbone.bottom_up.pretrained = "mcbeit_pretrained/checkpoint-best.pth" # mae pretrained weight does not need convert
model.backbone.bottom_up.stop_grad_conv1 = False
model.backbone.bottom_up.sincos_pos_embed = False
model.backbone.bottom_up.zero_pos_embed= True # zero init pos embed
model.backbone.bottom_up.init_values = 0.1 # 0.1 for base, 1e-5 for large
model.backbone.bottom_up.beit_qkv_bias = True
model.backbone.bottom_up.shared_rel_pos_bias = False



train.output_dir = 'work_dirs/mask_rcnn_base_FPN_25ep_LSJ_ft_mc-beit'